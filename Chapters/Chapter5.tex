...
En este capítulo se presentan las principales conclusiones obtenidas del trabajo realizado y se plantean posibles líneas de trabajo futuro para continuar y ampliar el desarrollo.
...
En esta sección, se exponen las conclusiones generales de los modelos desarrollados en el trabajo. Se analiza el desarrollo y los resultados alcanzados para la estimación de Potencia Límite de Canal (PLC) y la predicción del Flujo Crítico de Calor (CHF).
...
Definido el modelo de baja dimensionalidad, se realizó un análisis progresivo de las arquitecturas de multifidelidad y de la adición de información física. En este análisis se tomó como punto de referencia una red neuronal simple. Adicionalmente, para analizar la robustez de los modelos, se entrenaron a los modelos en el rango superior de flujo másico y se los evaluó en el rango inferior. Esto implica entrenar a los modelos en la región menos demandante para el CHF en cuanto a condiciones termohidráulicas. Se validó la arquitectura de multifidelidad y la adición de relaciones físicas con múltiples métricas, tanto estadísticas como específicas del estudio del CHF.

Luego, se analizó la adición de variables de entrada a los modelos. Estas variables son adicionales a las clásicamente utilizadas en la predicción de CHF, aunque su adición busca capturar efectos por fuera de la localidad del fenómeno. Se obtuvo un resultado desfavorable al utilizarlas, por lo que se descartó su adición a los modelos finales.

En última instancia, se entrenaron dos modelos de multifidelidad con y sin adición de información física. Este entrenamiento se realizó sobre todo el rango de los ensayos experimentales de la Universidad de Columbia, con un conjunto para evaluación reservado del entrenamiento. Se obtuvo un \dnbrmin{} de $1.113$ para el modelo \mfdnn{} y de $1.084$ para el modelo \mfpinn{}. Estos resultados son superadores a la metodología actual, con un \dnbrmin{} de $1.212$.

Finalmente, se utilizó el valor de \dnbrmin{} de la \mfpinn{} para una estimación de primer orden del impacto en el cálculo de la PLC por DNB. Para la ZH 5 se obtuvo un $3\%$ adicional de PLC para el caso de diseño. Esta mejora indica el beneficio de utilizar metodologías modernas para la predicción de CHF, al obtener resultados consistentes y robustos con la posibilidad de márgenes de seguridad más acotados.

\section{Próximos pasos}
Como trabajos futuros para continuar el desarrollo del modelo de estimación de DNBR y fracción de vacío en los canales refrigerantes de la \cna{} se proponen:
\begin{itemize}
    \item Analizar la capacidad del modelo de reproducir los peores casos de una estrategia de recambio de combustibles y casos anómalos en ella.
    ...
    \item Aplicar el modelo a un entorno de cálculo de estrategia de recambio de combustibles como cálculo en línea de la PLC. Esto permite reemplazar el uso de un límite fijo por ZH para toda la estrategia.
    \item Incorporar el modelo para el cálculo de PLC en línea realizado por AXCNA en la computadora de procesos de la \cna{}. Este código es utilizado para la calibración del algoritmo de la vigilancia de la limitación de potencia.
\end{itemize}
...
\begin{itemize}
    ...
    \item Realizar la optimización con Optuna de la \hfdnn{} de los modelos \mfdnn{} y \mfpinn{}.
    ...
    \item Analizar el impacto final sobre el diseño termohidráulico con el modelo \mfpinn{} implementado en COBRA3-CP.
\end{itemize}
